...
VIDEO_PATH = r"C:\Users\afons\Documents\SwProjects\object-detection\videos\5.mp4"
    ...
    if not is_paused:
        ...
        for r in results:
            for box in r.boxes:
                ...
                cvzone.putTextRect(
                frame,
                f"{label} #{track_id}: {confidence}",  # shows the unique ID
                (max(0, x1), max(35, y1)),
                scale=BBOX_TEXT_SCALE,
                thickness=BBOX_TEXT_THICKNESS,
                )

        cvzone.putTextRect(frame, f"Total {TRACKED_CLASS}s seen: {len(seen_ids)}", (20, 40)) # Display total unique count in the top-left corner
